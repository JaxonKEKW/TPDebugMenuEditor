...
public partial class Form1 : DarkForm
{
    ...
    public static class Prompt
    {
        public static string ShowDialog(string text, string caption, string textBoxDefault)
        {
            ...
            DarkLabel textLabel = new DarkLabel() { Left = 50, Top = 20, Text = text };
            DarkTextBox textBox = new DarkTextBox() { Left = 50, Top = 50, Width = 400, Text = textBoxDefault, };
            DarkLabel textLabel2 = new DarkLabel() { Left = 350, Top = 20, Text = "Bytes: " + Encoding.GetEncoding(932).GetByteCount(textBox.Text).ToString() };
            DarkButton confirmation = new DarkButton() { Text = "Ok", Left = 350, Width = 100, Top = 80, DialogResult = DialogResult.OK };
            var maxBytes = 64;
            ...
            textLabel2.ForeColor = Color.FromArgb(128, 255, 128);
        ...
        public static string ShowDialogInt(string text, string caption, int updownDefault)
        {
            ...
            DarkLabel textLabel = new DarkLabel() { Left = 50, Top = 20, Text = text };
            DarkNumericUpDown updown = new DarkNumericUpDown() { Left = 50, Top = 50, Width = 400, Value = updownDefault, Maximum = 255, Hexadecimal = true };
            DarkButton confirmation = new DarkButton() { Text = "Ok", Left = 350, Width = 100, Top = 80, DialogResult = DialogResult.OK };
            var maxBytes = 3;
        ...
        if (ran == true)
        {
            if (dataGridView1.CurrentCell.ColumnIndex >= 3)
            {
                ...
                while (SuccessfullyChanged == false)
                {
                    ...
                    {
                        saveToolStripButton.Enabled = false;
                        DarkMessageBox.ShowError("New value is larger than 64 bytes (MAX).", "Error: File size increased");
                    }
                    else if (Encoding.GetEncoding(932).GetByteCount(editCellValue) <= 3)
                    {
    ...
    private void openToolStripButton_Click(object sender, EventArgs e)
    {
        ...
        if (ran == true)
        {
            DialogResult saveResult = DarkMessageBox.ShowWarning("Would you like to save your changes before opening a new file?", "Save changes?", DarkDialogButton.YesNoCancel);
    ...
    private void Form1_FormClosing(object sender, FormClosingEventArgs e)
    {
        ...
        if (ran == true)
        {
            saveResult = DarkMessageBox.ShowWarning("Do you wish to save your changes before exiting?", "Save changes?", DarkDialogButton.YesNoCancel);
            if (saveResult == DialogResult.Yes)
            {
    ...
    private void switchtoLightMode_Click(object sender, EventArgs e)
    {
        //DialogResult lightmodeResult = DarkMessageBox.ShowWarning("This will RESTART the program. You will lose unsaved progress. Are you sure you want to switch to Light Mode?", "Warning", DarkDialogButton.YesNo);
        //if (lightmodeResult == DialogResult.Yes)
        //{
        //    try
        //    {
        //        reader.Close();
